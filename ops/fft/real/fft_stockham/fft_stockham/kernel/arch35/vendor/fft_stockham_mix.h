 ...
 */
// Shared self-sorting mixed-radix Stockham FFT device kernels for Ascend 950
// (arch35). These are the same high-performance per-radix-specialized,
// autosort, UB-cached-DFT routines used by the c2c arch35 mixed kernel
// (see ops/fft/c2c/arch35/.../fft_c2c_mix_multi_core.cpp). They are reused by
// the R2C and C2R arch35 kernels so the heavy complex-FFT core matches the
// optimized c2c implementation instead of the slow generic radix loop plus
// separate transpose passes.
//
// Layout expected from the host coefficient builders (DIT Stockham):
//   dftMatrix : per stage radix*radix complex, entry (q*radix+p) interleaved
//               (re,im) = exp(sign*2*pi*p*q/radix)
//   twMat     : per stage radix*prev complex, entry (p*prev+j) interleaved
//               (re,im) = exp(sign*2*pi*p*j/len), len = prev*radix
// Both arrays are concatenated across stages in radix-list order, and the
// driver advances the offsets internally (dftOffset += radix*radix,
// twOffset += radix*prev).

#ifndef ASDOPS_FFT_STOCKHAM_MIX_H
#define ASDOPS_FFT_STOCKHAM_MIX_H
...
constexpr int STK_THREAD_NUM_RAD2 = 1024;
constexpr int STK_THREAD_NUM_RAD3 = 512;
constexpr int STK_THREAD_NUM = 256;

template <bool SCALE_OUT, bool TRANSPOSE>
__simt_vf__ __launch_bounds__(STK_THREAD_NUM_RAD2) inline void stockhamForwardRadix2(
    __gm__ float* in, __gm__ float* twMat, __gm__ float* out, int batch, int M, int N, int batchOffset, int len,
...
template <bool SCALE_OUT, bool TRANSPOSE>
__simt_vf__ __launch_bounds__(STK_THREAD_NUM_RAD3) inline void stockhamForwardRadix3(
    __gm__ float* in, __gm__ float* twMat, __gm__ float* dftMat, __gm__ float* out, int batch, int M, int N,
    int batchOffset, int len,
...
template <bool SCALE_OUT, bool TRANSPOSE>
__simt_vf__ __launch_bounds__(STK_THREAD_NUM) inline void stockhamForwardRadix5(__gm__ float* in, __gm__ float* twMat,
...
template <bool SCALE_OUT, bool TRANSPOSE>
__simt_vf__ __launch_bounds__(STK_THREAD_NUM) inline void stockhamForwardRadix7(__gm__ float* in, __gm__ float* twMat,
                                                                                __gm__ float* dftMat, __gm__ float* out,
                                                                                int batch, int M, int N,
                                                                                int batchOffset, int len,
...
constexpr int STK_jTILE = 64;

template <bool SCALE_OUT, bool TRANSPOSE>
__simt_vf__ __launch_bounds__(STK_THREAD_NUM) inline void stockhamForwardGeneral(__gm__ float* in, __gm__ float* twMat,
                                                                                 __gm__ float* dftMat,
                                                                                 __gm__ float* out, int batch, int M,
                                                                                 int N, int batchOffset, int len,
                                                                                 int radix, int prev, int part)
{
    __ubuf__ float dft[2][20][20];
    __ubuf__ float tw[2][20][STK_jTILE];
    ...
    for (int jBase = 0; jBase < prev; jBase += STK_jTILE) {
        int tail = (prev - jBase < STK_jTILE) ? (prev - jBase) : STK_jTILE;
...
// Runs the full self-sorting mixed-radix Stockham forward FFT over `localBatch`
// contiguous length-`fftLen` complex signals already laid out in `ws0`
// (interleaved re/im). Ping-pongs between `ws0` and `ws1`; returns the buffer
// that holds the natural-order result. `radixArr` lists the per-stage radices
// in the same order the host concatenated the dft/tw coefficient blocks.
inline __aicore__ __gm__ float* runStockhamForward(__gm__ float* ws0, __gm__ float* ws1, __gm__ float* dftMatrix,
                                                   __gm__ float* twMatrix, const __gm__ int32_t* radixArr,
                                                   int radixListLen, int localBatch, int fftLen)
{
    int len = 1;
    int64_t twOffset = 0;
    int64_t dftOffset = 0;

    for (int stage = 0; stage < radixListLen; ++stage) {
        int radix = radixArr[stage];
        int prev = len;
        len *= radix;
        int part = fftLen / radix;

        __gm__ float* stageInput = (stage & 1) ? ws1 : ws0;
        __gm__ float* stageOutput = (stage & 1) ? ws0 : ws1;

        __gm__ float* twStage = twMatrix + twOffset * 2;
        __gm__ float* dftStage = dftMatrix + dftOffset * 2;

        AscendC::PipeBarrier<PIPE_V>();

        if (radix == 2) {
            asc_vf_call<stockhamForwardRadix2<false, false>>(dim3(STK_THREAD_NUM_RAD2), stageInput, twStage,
                                                             stageOutput, localBatch, 1, fftLen, 0, len, radix, prev,
                                                             part);
        } else if (radix == 3) {
            asc_vf_call<stockhamForwardRadix3<false, false>>(dim3(STK_THREAD_NUM_RAD3), stageInput, twStage, dftStage,
                                                             stageOutput, localBatch, 1, fftLen, 0, len, radix, prev,
                                                             part);
        } else if (radix == 5) {
            asc_vf_call<stockhamForwardRadix5<false, false>>(dim3(STK_THREAD_NUM), stageInput, twStage, dftStage,
                                                             stageOutput, localBatch, 1, fftLen, 0, len, radix, prev,
                                                             part);
        } else if (radix == 7) {
            asc_vf_call<stockhamForwardRadix7<false, false>>(dim3(STK_THREAD_NUM), stageInput, twStage, dftStage,
                                                             stageOutput, localBatch, 1, fftLen, 0, len, radix, prev,
                                                             part);
        } else {
            asc_vf_call<stockhamForwardGeneral<false, false>>(dim3(STK_THREAD_NUM), stageInput, twStage, dftStage,
                                                              stageOutput, localBatch, 1, fftLen, 0, len, radix, prev,
                                                              part);
        }

        twOffset += static_cast<int64_t>(radix) * prev;
        dftOffset += static_cast<int64_t>(radix) * radix;
    }

    return (radixListLen & 1) ? ws1 : ws0;
}

} // namespace stockham
} // namespace AsdSip

#endif // ASDOPS_FFT_STOCKHAM_MIX_H
